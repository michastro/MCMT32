...
type
  TSetupTelescope = class(TForm)
    ...
    procedure Edit_Reduc_ADChange(Sender: TObject);
    procedure Edit_PasParTour_ADChange(Sender: TObject);
    procedure Edit_Reduc_DECChange(Sender: TObject);
    procedure Edit_PasParTour_DECChange(Sender: TObject);
    procedure Edit_MPasParPas_ADChange(Sender: TObject);
    procedure Edit_MPasParPas_DECChange(Sender: TObject);
          ...
          begin
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Corr_P_AD.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesCorrection)
            else
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Corr_M_AD.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesCorrection)
            else
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Point_L_AD.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesPoint_L)
            else
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Point_R_AD.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesPoint_R)
            else
            ...
            if not(MCMT2021) then
            begin
              ComboBox_Accel_AD.ItemIndex := ord(receivestring[Accel_EEprom + 1])+2;
            end
            else
            begin
              case ord(receivestring[Accel_EEprom+1]) of
                1,2,3,4: ComboBox_Accel_AD.ItemIndex := 6-ord(receivestring[Accel_EEprom+1]);
                8: ComboBox_Accel_AD.ItemIndex:=1;
                12: ComboBox_Accel_AD.ItemIndex:=0;
              end;
            end;
            // DIR_AD en 21
            ...
            //  MicroPas par pas en 22 23
            ValueInt:=ord(receivestring[MicroStep + 2]) * 256 +
              ord(receivestring[MicroStep + 1]);
            if ValueInt>4000 then
            begin
              ValueInt:=128;
              astring := 'L' + AnsiChar(MicroStep) + AnsiChar(ValueInt mod 256) +
                AnsiChar(0) + AnsiChar(0);
                sendcommand(astring, 0, C_Alpha);
              while deviceready(C_Alpha) <> 1 do;
              astring := 'L' + AnsiChar(MicroStep+1) + AnsiChar(ValueInt div 256) +
                AnsiChar(0) + AnsiChar(0);
              sendcommand(astring, 0, C_Alpha);
              while deviceready(C_Alpha) <> 1 do;
            end;
            Edit_MPasParPas_AD.Text := IntToStr(ValueInt);

            //  Pas par Tour en 24 25
            ValueInt:=ord(receivestring[Step + 2]) * 256 +
              ord(receivestring[Step + 1]);
            if ValueInt>4000 then
            begin
              ValueInt:=200;
              astring := 'L' + AnsiChar(Step) + AnsiChar(ValueInt mod 256) +
                AnsiChar(0) + AnsiChar(0);
                sendcommand(astring, 0, C_Alpha);
              while deviceready(C_Alpha) <> 1 do;
              astring := 'L' + AnsiChar(Step+1) + AnsiChar(ValueInt div 256) +
                AnsiChar(0) + AnsiChar(0);
              sendcommand(astring, 0, C_Alpha);
              while deviceready(C_Alpha) <> 1 do;
            end;
            SpeedIntMCMT.Step_AD:=ValueInt;
            Edit_PasParTour_AD.Text := IntToStr(SpeedIntMCMT.Step_AD);
            ...
            Reduc_AD:=Reduc_AD+ValueInt/65536;
            if ValueInt = 0
            ...
            Resol_AD := Trunc(SpeedIntMCMT.Microstep_AD * Reduc_AD*SpeedIntMCMT.Step_AD);
          ...
          begin
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Guidage_DEC.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesGuidage)
            else
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Corr_P_DEC.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesCorrection)
            else
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Corr_M_DEC.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesCorrection)
            else
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Point_L_DEC.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesPoint_L)
            else
            ...
            then
              if not(MCMT2021) then
                ...
                Edit_V_Point_R_DEC.text := FloatToStrF(150000000 / (ValueLong+1),
                  ffFixed, 8, NbreDecimalesPoint_R)
            else
            ...
            if not(MCMT2021) then
            begin
              ComboBox_Accel_DEC.ItemIndex := ord(receivestring[Accel_EEprom + 1])+2;
            end
            else
            begin
              case ord(receivestring[Accel_EEprom+1]) of
                1,2,3,4: ComboBox_Accel_DEC.ItemIndex := 6-ord(receivestring[Accel_EEprom+1]);
                8: ComboBox_Accel_DEC.ItemIndex:=1;
                12: ComboBox_Accel_DEC.ItemIndex:=0;
              end;
            end;
            ...
            Edit_MPasParPas_DEC.Text := IntToStr(ValueInt);
            ...
            SpeedIntMCMT.Step_DEC:=ValueInt;
            Edit_PasParTour_DEC.Text := IntToStr(SpeedIntMCMT.Step_DEC);
            ...
            ValueInt := ord(receivestring[Reduc_Frac_EEprom + 1]) * 256 +
              ord(receivestring[Reduc_Frac_EEprom + 2]);
            Reduc_DEC:=Reduc_DEC+ValueInt/65536;
          if ValueInt = 0
            then // Le coefficient de réduction est entier ...
            begin
            ...
            Resol_DEC := Trunc(SpeedIntMCMT.Microstep_DEC * Reduc_DEC*SpeedIntMCMT.Step_DEC);
    ...
    begin
      try
        if not(MCMT2021) then
          ...
          CalcValue := round((150000000 / StrToFloat(bstring)+1))
      except
        CalcValue := 0;
      end;
    end
  ...
  try
    ...
    try
      ...
      begin
        ...
        if MCMT2021 then
        begin
          case ComboBox_Accel_AD.ItemIndex of
            0: ValueInt:=12;
            1: ValueInt:=8;
            2,3,4,5: ValueInt:=6-ComboBox_Accel_AD.ItemIndex;
          end;
        end
        else
        begin
          case ComboBox_Accel_AD.ItemIndex of
            0,1,2: ValueInt:=0;
            3,4,5: ValueInt:=ComboBox_Accel_AD.ItemIndex-2;
          end;
        end;
        ...
        // Ecriture de MicroPas par pas en 22 23
        ...
        // Ecriture de Pas par Tour en 24 25
        ...
        Resol_AD := ValueLong;
        ValueInt := Resol_AD div (StrToInt(Edit_PasParTour_AD.text)*StrToInt(Edit_MPasParPas_AD.text));
        ...
        ValueFloat := frac((StrToFloat(Edit_Reduc_AD.text)))*65536;
        ValueInt:=Round(ValueFloat);
        MSB := (ValueInt shr 8) and $FF;
        LSB := ValueInt and $FF;
        ...
        if MCMT2021 then
        begin
          case ComboBox_Accel_DEC.ItemIndex of
            0: ValueInt:=12;
            1: ValueInt:=8;
            2,3,4,5: ValueInt:=6-ComboBox_Accel_DEC.ItemIndex;
          end;
        end
        else
        begin
          case ComboBox_Accel_DEC.ItemIndex of
            0,1,2: ValueInt:=0;
            3,4,5: ValueInt:=ComboBox_Accel_DEC.ItemIndex-2;
          end;
        end;
        astring := 'L' + AnsiChar(Accel_EEprom) + AnsiChar(ValueInt) +
          AnsiChar(0) + AnsiChar(0);
        ...
        // Ecriture de MicroPas par pas en 22 23
        ValueInt := StrToInt(Edit_MPasParPas_DEC.Text);
        if ValueInt = -1 //
        then
          ValueInt := 0;
        astring := 'L' + AnsiChar(MicroStep) + AnsiChar(ValueInt mod 256) +
        ...
        // Ecriture de Pas par Tour en 24 25
        ValueInt := StrToInt(Edit_PasParTour_DEC.Text);
        if ValueInt = -1 //
        then
          ValueInt := 0;
        astring := 'L' + AnsiChar(Step) + AnsiChar(ValueInt mod 256) +
        ...
        Resol_DEC := ValueLong;
        ValueInt := Resol_DEC div (StrToInt(Edit_PasParTour_DEC.text)*StrToInt(Edit_MPasParPas_DEC.text));
        MSB := (ValueInt shr 8) and $FF;
        LSB := ValueInt and $FF;
        astring := 'L' + AnsiChar(Reduc_Trunc_EEprom) + AnsiChar(MSB) +
          AnsiChar(0) + AnsiChar(0);
        ...
        ValueFloat := frac((StrToFloat(Edit_Reduc_DEC.text)))*65536;
        ValueInt:=Round(ValueFloat);
        MSB := (ValueInt shr 8) and $FF;
        LSB := ValueInt and $FF;
        astring := 'L' + AnsiChar(Reduc_Frac_EEprom) + AnsiChar(MSB) +
...
begin
  ...
  if (Flatitude > 0) //
    then
    begin
      SpinEdit_Declinaison_Polaire_Limite_Fourche.MaxValue := 90;
      SpinEdit_Declinaison_Polaire_Limite_Fourche.MinValue := 0
    end
    else
    begin
      SpinEdit_Declinaison_Polaire_Limite_Fourche.MaxValue := 0;
      SpinEdit_Declinaison_Polaire_Limite_Fourche.MinValue := -90
    end;
end;

procedure TSetupTelescope.Edit_MPasParPas_ADChange(Sender: TObject);
var MPasParPas:integer;
begin
  if InitPossible then
  begin
    try
      MPasParPas := strtoInt(Edit_MPasParPas_AD.text);
    except
      Edit_MPasParPas_AD.text:='128';
      MPasParPas :=128;
    end;
    if MPasParPas=0 then
    begin
      MPasParPas:=128;
      Edit_MPasParPas_AD.text:='128';
    end;
    ...
    Resol_AD := Trunc(Reduc_AD *StrToInt(Edit_PasParTour_AD.text)*StrToInt(Edit_MPasParPas_AD.text));
...
procedure TSetupTelescope.Edit_MPasParPas_DECChange(Sender: TObject);
var MPasParPas:integer;
begin
 if InitPossible then
  begin
    try
      MPasParPas := strtoInt(Edit_MPasParPas_DEC.text);
    except
      Edit_MPasParPas_DEC.text:='128';
      MPasParPas :=128;
    end;
    if MPasParPas=0 then
    begin
    ...
    Resol_DEC := Trunc(Reduc_DEC *StrToInt(Edit_PasParTour_DEC.text)*StrToInt(Edit_MPasParPas_DEC.text));
    ...
    Edit_V_Corr_P_DEC.text :=
      FloatToStrF(Vitesse_DEC * strtofloat(Edit_V_Corr_P_DEC_Calc.text), ffFixed, 8,
      NbreDecimalesCorrection);
    Edit_V_Corr_M_DEC.text :=
      FloatToStrF(Vitesse_DEC * strtofloat(Edit_V_Corr_M_DEC_Calc.text), ffFixed, 8,
      NbreDecimalesCorrection);
    Edit_V_Point_L_DEC.text := FloatToStrF(strtofloat(Edit_V_Point_L_DEC_Calc.text)
      / 360 * Resol_DEC, ffFixed, 8, NbreDecimalesPoint_L);
    Edit_V_Point_R_DEC.text := FloatToStrF(strtofloat(Edit_V_Point_R_DEC_Calc.text)
      / 360 * Resol_DEC, ffFixed, 8, NbreDecimalesPoint_R);
  end;
end;

procedure TSetupTelescope.Edit_PasParTour_ADChange(Sender: TObject);
var PasParTour:integer;
begin
  if InitPossible then
  begin
    try
      PasParTour := strtoInt(Edit_PasParTour_AD.text);
    except
      Edit_PasParTour_AD.text:='1';
      PasParTour :=1;
    end;
    if PasParTour=0 then
    begin
      PasParTour:=1;
      Edit_PasParTour_AD.text:='1';
    end;
...
procedure TSetupTelescope.Edit_PasParTour_DECChange(Sender: TObject);
var PasParTour:integer;
begin
  if InitPossible then
  begin
    try
      PasParTour := strtoInt(Edit_PasParTour_DEC.text);
    except
      Edit_PasParTour_DEC.text:='1';
      PasParTour :=1;
    end;
    if PasParTour=0 then
    begin
...
procedure TSetupTelescope.Edit_Reduc_ADChange(Sender: TObject);
begin
  if InitPossible then
  begin
    try
...
procedure TSetupTelescope.Edit_Reduc_DECChange(Sender: TObject);
begin
  if InitPossible then
  begin
    try
...
begin
  Resol_Calc := strtofloat(Edit_Reduc_AD.text) * StrToInt(Edit_PasParTour_AD.text)*StrToInt(Edit_MPasParPas_AD.text);
...
//  Resol_Calc := strtofloat(Edit_Reduc_DEC.text) * SpeedIntMCMT.MPasParTour_DEC;
  Resol_Calc := strtofloat(Edit_Reduc_DEC.text) * StrToInt(Edit_PasParTour_DEC.text)*StrToInt(Edit_MPasParPas_DEC.text);
  V_Calc := Resol_Calc / Jour_Sideral; // en µPas/s
...
//  Resol_AD := Trunc(Reduc_AD * SpeedIntMCMT.MPasParTour_AD);
  Resol_AD := Trunc(Reduc_AD * StrToInt(Edit_PasParTour_AD.text)*StrToInt(Edit_MPasParPas_AD.text));
...
//  Resol_DEC := Trunc(Reduc_DEC * SpeedIntMCMT.MPasParTour_DEC);
  Resol_DEC := Trunc(Reduc_DEC * StrToInt(Edit_PasParTour_DEC.text)*StrToInt(Edit_MPasParPas_DEC.text));
...
begin
  // Axe ALPHA
  V_Max := V_Guidage_Max / strtofloat(Edit_Reduc_AD.text) * SpeedIntMCMT.MPasParTour_AD*
    Jour_Sideral;
...
begin
  // Axe ALPHA
  V_Max := V_Guidage_Max / strtofloat(Edit_Reduc_DEC.text) * SpeedIntMCMT.MPasParTour_DEC *
    Jour_Sideral;
